...
export const Bridge = ({ sidebarSubject }: Props) => {
    ...
    const { currentChainId, changeNetwork, getTokenAvailableInPool, getTransferContractAdd, deposit } = useContract(contractAddress);
    const { getWalletTokenAmount, getAmountCanTranfer, approveAmountTransfer } = useToken(coin.address)
    ...
    const getTokenAmount = () => {
        if (!contractAddress || !walletAddress || currentChainId?.toString() != chainIn.chainId) {
    ...
    const getSubmitButton = () => {
        ...
        if (currentChainId?.toString() != chainIn.chainId) {
            return (
                <div className=" mt-2 w-full py-3 text-center bg-orange-500/90 rounded-xl text-white font-medium text-xl cursor-pointer" onClick={() => { changeNetwork(chainIn) }}>Change Metamask Network</div>
            )
        }

        return (
            <div className=" mt-2 w-full py-3 text-center bg-orange-500/90 rounded-xl text-white font-medium text-xl cursor-pointer" onClick={onBridge}>Bridge</div>
        )
    }
        ...
        <div className="w-full h-full justify-center flex bg-slate-900">
            <div className="border-solid border-2 border-stone-500 rounded-2xl mt-44 h-fit w-[50rem] m-0 p-4">
                <div className=" flex flex-row justify-between items-center">
                    <BridgeDropdown value={chainIn} onPress={setChainInput} options={Data.chain} />
                    <BsArrowRight color="white" size={"2rem"} />
                    <BridgeDropdown value={chainOut} onPress={setChainOut} options={Data.chain} />
                </div>
                <div className="w-full border border-slate-600 rounded-xl mt-4">
                    <div className=" flex flex-row items-center w-full pr-4">
                        ...
                        <input className=" outline-none bg-transparent text-lg text-white caret-slate-100 flex-1" onChange={onChangeAmountInput} value={amount} placeholder="0" />
                        <div className="rounded-full flex flex-row items-center bg-orange-600 p-2 cursor-pointer relative" onClick={() => setOpenToken(!isOpenToken)}>
                            <div className="text-slate-200 font-medium mx-1 cursor-pointer">{coin.name}</div>
                            <FaChevronDown color="white" />
                            {
                                isOpenToken && <div className=" absolute -translate-x-1/2 px-6 top-full mt-2 rounded-2xl border bg-orange-500 left-1/2">
                                    {Data.coin[chainIn.chainId].map(coin => {
                                        return (
                                            <div className=" py-2 text-white font-medium text-lg" onClick={() => onSelectCoin(coin)}>{coin.name}</div>
                                        )
                                    })}
                                </div>
                            }
                    ...
                    {tokenAmountMax && <div className=" text-slate-400 w-full text-right pr-[14px] mb-2">Balance: {tokenAmountMax}</div>}
                </div>
                {tokenAvaible && <div className=" w-full text-right text-slate-300 pr-4 mt-2">Token available in Pool: {tokenAvaible}</div>}
                {getSubmitButton()}
            </div>
